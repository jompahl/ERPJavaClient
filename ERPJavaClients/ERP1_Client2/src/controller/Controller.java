...
public class Controller implements Initializable {
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		columnID.setCellValueFactory(data -> new SimpleStringProperty(data.getValue().getNo()));
		columnFirst.setCellValueFactory(data -> new SimpleStringProperty(data.getValue().getFirstName()));
		columnLast.setCellValueFactory(data -> new SimpleStringProperty(data.getValue().getLastName()));
		columnJob.setCellValueFactory(data -> new SimpleStringProperty(data.getValue().getJobTitle()));
		columnAddress.setCellValueFactory(data -> new SimpleStringProperty(data.getValue().getAddress()));
		columnPhoneNbr.setCellValueFactory(data -> new SimpleStringProperty(data.getValue().getPhoneNumber()));
		...
		if(no.isEmpty() || firstName.isEmpty() || lastName.isEmpty() || job == null || address.isEmpty() || phoneNbr.isEmpty()) {
			...
			try {
				if(proxy.createEmployee(no, firstName, lastName, job, address, phoneNbr)) {
					...
					responseLabel.setTextFill(Color.RED);
					responseLabel.setText("Employee: " + no + " already exists");
				}
			} catch (RemoteException e) {
				e.printStackTrace();
			}
